...
- 시그니처 커서: `cursor: url(/cursors/signature.cur) 2 2, default` (fallback: `default`)
- 시그니처 커서 스펙: 32×32, 로즈 핑크 화살표, hotspot (2, 2), 스튜디오에서 직접 제작
- 커서 파일 미완성 시: 시스템 커서 유지 (기능에 영향 없음)
...
- 드롭 시: `File` 객체를 모듈 레벨 변수(`landingFile`)에 저장, `router.push('/studio?fromLanding=true')`
- **파일 전달 방식:** `sessionStorage` 대신 in-memory 공유 (base64 크기 제한 회피). `lib/landingStore.ts`에 `setLandingFile(file)` / `getLandingFile()` / `clearLandingFile()` 함수
- **파일 크기 제한:** 10MB 초과 시 드롭 거부 + 에러 메시지 (백엔드 제한과 동일)

### Studio Integration
- 스튜디오 페이지에서 `useEffect`로 `fromLanding` 쿼리 파라미터 감지
- `getLandingFile()`로 파일 로드 → `selectFile(file)` 호출 → 업로드 흐름 자동 시작
- 로드 후 `clearLandingFile()` + URL에서 쿼리 파라미터 제거 (`router.replace('/studio')`)
- 파일이 없는 경우 (직접 URL 접근): 무시하고 일반 idle 상태

### Mobile
- 파티클 비활성 (`window.matchMedia('(pointer: fine)')`)
- CSS 빛 효과만 유지
- 드롭존 비활성, CTA 버튼만 (단순 `/studio` 링크, 파일 전달 없음)
...
- `page.tsx`: Server Component (async, SSR). 메타데이터 + JSON-LD + 정적 콘텐츠 렌더링
- `Hero.tsx`: `"use client"`. mousemove, drag/drop, canvas 처리
- `HowItWorks.tsx`: Server Component 가능 (인터랙션 없음)
- `FAQ.tsx`: Server Component (SEO를 위해 SSR 필수). `<details>/<summary>` 네이티브
- `Footer.tsx`: Server Component
- `ParticleCanvas.tsx`: `"use client"`. Canvas 2D 파티클 렌더링

## Theme Support

- P1에서는 다크 테마 기준으로 구현
- 라이트 테마: 빛 효과를 딥 로즈(`var(--color-accent)`)로 유지하되 opacity 낮춤 (0.08)
- 파티클: 다크에서는 `var(--color-accent)` 기반, 라이트에서는 `var(--color-accent-hover)` 기반
- 시그니처 커서: 양쪽 배경에서 잘 보이는 디자인 (로즈 핑크 + 흰 테두리)
...
### First 200 Words (KO, GEO)
"Pointint는 이미지를 나만의 Windows 커서로 만드는 제작 도구입니다. PNG, JPG, WebP 이미지를 업로드하면 배경이 자동으로 제거되고, 위치와 핫스팟을 편집하고, Normal, Text, Link 3가지 상태로 시뮬레이션한 뒤, .cur 파일과 원클릭 설치 파일을 다운로드할 수 있습니다."

### Technical
- `sitemap.xml` 자동 생성 (Next.js)
- `robots.txt`
- `hreflang`: 현재 cookie 기반 locale이라 path 기반 분리 없음. `<link rel="alternate" hreflang="ko" href="https://pointtint.com">` 단일 URL + `x-default`
- FAQ는 Server Component로 렌더링 → 초기 HTML에 포함 → 크롤러 접근 가능
...
| `frontend/src/lib/landingStore.ts` | 새로 생성 — 랜딩→스튜디오 파일 전달 |
| `frontend/public/cursors/signature.cur` | 시그니처 커서 (없으면 시스템 커서 fallback) |
| `frontend/src/app/studio/page.tsx` | fromLanding 파일 로드 로직 |
| `frontend/public/og-image.png` | OG 이미지 (1200×630) |
